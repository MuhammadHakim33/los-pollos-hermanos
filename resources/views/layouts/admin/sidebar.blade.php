...
<div x-cloak :class="sidebarOpen ? 'translate-x-0 ease-out' : '-translate-x-full ease-in'" class="fixed inset-y-0 left-0 z-30 w-64 overflow-y-auto transition duration-300 transform bg-gray-900 lg:translate-x-0 lg:static lg:inset-0">
    <div class="flex items-center justify-center mt-8">
        <div class="flex items-center">
            <img alt="RM. Selera Panca" class="mr-4 rounded-xl size-20" src="{{ asset('images/Logo RM. Selera Panca.png') }}"></img>
        </div>
    </div>

    <nav class="mt-10 flex flex-col h-screen">
        ...
        <a class="flex items-center px-6 py-2 mt-4 {{ Route::currentRouteNamed('admin.menu') ? 'text-gray-100' : 'text-gray-500' }} hover:bg-gray-700 hover:bg-opacity-25 hover:text-gray-100" href="{{ route('admin.menu') }}">
            <svg class="w-6 h-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path d="M18 18V20H6V18H18ZM21 11V13H3V11H21ZM18 4V6H6V4H18Z"></path>
            </svg>

            <span class="mx-3">Manajemen Menu</span>
        </a>
        ...
        <a class="flex items-center px-6 py-2 mt-4 {{ Route::currentRouteNamed('admin.pesanan') ? 'text-gray-100' : 'text-gray-500' }} hover:bg-gray-700 hover:bg-opacity-25 hover:text-gray-100" href="{{ route('admin.pesanan') }}">
            ...
            <span class="mx-3">Manajemen Pesanan</span>
        </a>

        <a class="flex grow items-center px-6 py-2 mt-4 {{ Route::currentRouteNamed('admin.logout') ? 'text-gray-100' : 'text-gray-500' }} hover:bg-gray-700 hover:bg-opacity-25 hover:text-gray-100" href="{{ route('admin.logout') }}">
            <svg class="w-6 h-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path
